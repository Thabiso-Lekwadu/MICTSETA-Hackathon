...
FastAPI backend for the Northern Cape Fleet Dispatch system. Loads the graph bundle
produced by `kedro run` (data/03_primary/nc_road_graph.pkl), ingests vehicle telemetry
from TWO interchangeable sources (a real Traccar Client mobile app feed, or a built-in
...
This app sits outside the Kedro pipeline DAG on purpose: it's a long-running live
service with request-scoped state, not a batch data transformation, so it doesn't
belong in pipelines/. It consumes the pipeline's output artifact directly and shares
its spatial-indexing and routing-weight logic with pipelines/route_reporting via
routing.py, so "how a route is scored" is defined in exactly one place for both the
batch report and the live demo.
...
from nc_freight_optimizer.road_network_core import TOWNS
...
# Configuration
# ---------------------------------------------------------------------------
# This file lives at src/nc_freight_optimizer/apps/live_backend.py; the project
# root (where data/ lives) is three levels up regardless of the working directory
# the server is launched from.
PROJECT_ROOT = Path(__file__).resolve().parents[3]
GRAPH_PATH = PROJECT_ROOT / "data" / "03_primary" / "nc_road_graph.pkl"
...
# Matches the roughness-multiplier scale established in conf/base/parameters.yml.
...
# ---------------------------------------------------------------------------
# Startup: load the pipeline-produced graph bundle, build indices, resolve destination
# ---------------------------------------------------------------------------
def load_graph_bundle() -> tuple[nx.Graph, set, dict[int, tuple[float, float]]]:
    if not GRAPH_PATH.exists():
        raise FileNotFoundError(
            f"'{GRAPH_PATH}' not found. Run 'kedro run' from the project root first "
            f"to produce it."
        )
...
live_topology, main_nodes, cluster_coord = load_graph_bundle()
...
destination_lon, destination_lat = TOWNS[DESTINATION_NAME]
...
def telematics_simulate_step() -> dict:
    ...
    logger.info(
        ...
        lap_position, latitude, longitude, payload["cargo_temp_c"],
    )
    return payload
