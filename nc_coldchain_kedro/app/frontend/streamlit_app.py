...
with tab_tracker:
    ...
    if not st.session_state.trip_configured:
        ...
        # Newer Streamlit requires every container created outside a fragment to be
        # written to during the initial run, or a later fragment rerun raises
        # "container not written to during the initial run". Claim each slot now.
        for _slot in tracker_ui.values():
            _slot.empty()
...
with tab_weather:
    ...
    # Claim every slot during the initial run (see note in the tracker tab) so the
    # fragment can safely write to them on later reruns.
    for _wslot in wui.values():
        _wslot.empty()
