...
streamlit run app/live_frontend.py
"""
...
# The synthetic data generator now lives in the package's legacy/ folder. Add it
# to sys.path so `import vrp_simulation_generator` resolves wherever streamlit is
# launched from (otherwise the ingestion ticker shows "could not be imported").
import os as _os2
import sys as _sys2
_LEGACY_F = _os2.path.join(_os2.path.dirname(__file__), "..", "src", "nc_coldchain", "legacy")
if _os2.path.isdir(_LEGACY_F) and _LEGACY_F not in _sys2.path:
    _sys2.path.insert(0, _LEGACY_F)
try:
...
# ---------------------------------------------------------------------------
import os as _os
BACKEND_URL = _os.environ.get("BACKEND_URL", "http://127.0.0.1:8000")
POLL_INTERVAL_SECONDS = 2
REQUEST_TIMEOUT_SECONDS = 15  # weather-profile does live OpenWeatherMap lookups; 5s was too short
...
with tab_tracker:
    ...
    if not st.session_state.trip_configured:
        ...
        # Persistent placeholders created ONCE. Each is written a real initial value
        # below (that "claim" is what newer Streamlit needs), so the fragment can
        # then update ONLY their values every poll — nothing is rebuilt, so the
        # numbers just tick in place with no flicker.
        ...
        # initial claim render — writes every slot once during the main run
        tracker_ui["warning"].info("Connecting to live telemetry...")
        tracker_ui["vehicle"].metric("Active Vehicle ID", "-")
        tracker_ui["coords"].metric("Current GPS Coordinates", "-")
        tracker_ui["cargo_temp"].metric("Cargo Temperature", "-")
        tracker_ui["compressor"].metric("Compressor Load", "-")
        tracker_ui["status_banner"].caption(" ")
        tracker_ui["action_panel"].markdown(" ")
        ...
        tracker_ui["feed"].caption(" ")
        tracker_ui["updated"].caption(" ")

        @st.fragment(run_every=POLL_INTERVAL_SECONDS)
        def tracker_metrics_view():
            # Only UPDATE the existing slots' values — never recreate the layout, so
            # the section updates smoothly in place instead of blinking.
            ...
            if telematics.get("cargo_at_risk"):
                ...
                # Write the whole panel as ONE element (a single colored box with
                # the header + list) so we never call .container()/.empty() on an
                # outside slot inside the fragment — that update is in place, no flicker.
                _panel_msg = header + "\n\n" + "\n".join(f"- {it}" for it in items)
                (tracker_ui["action_panel"].error if level >= 3
                 else tracker_ui["action_panel"].warning)(_panel_msg)
            else:
                tracker_ui["action_panel"].markdown(" ")
        ...
        if st.button("🚨 Generate Cargo-Risk Mitigation Report (AI)", use_container_width=True, key="cargo_mitigation_btn"):
            ...
            if alert_result is not None:
                if alert_result.get("status") == "success":
                    st.session_state["mitigation_report"] = {
                        "markdown": alert_result.get("strategy_markdown", ""),
                        "model": alert_result.get("model", "local model"),
                    }
                else:
                    st.session_state["mitigation_report"] = {
                        "error": alert_result.get("message", "Mitigation report failed."),
                    }
        # Render the mitigation memo from session state on EVERY rerun, so clicking
        # the Business Strategy Report button (a separate rerun) never clears it.
        _mit = st.session_state.get("mitigation_report")
        if _mit:
            if _mit.get("error"):
                st.error(_mit["error"])
            else:
                st.markdown(_mit["markdown"])
                st.caption(f"Generated on-box by `{_mit['model']}`.")
        st.divider()
                ...
                if eval_result is not None:
                    if eval_result.get("status") == "success":
                        st.session_state["eval_report"] = {
                            "markdown": eval_result.get("strategy_markdown", ""),
                            "model": eval_result.get("model", "local model"),
                        }
                    else:
                        st.session_state["eval_report"] = {
                            "error": eval_result.get("message", "AI evaluation report failed."),
                        }
            # Render the simulation-vs-real evaluation from session state every rerun,
            # so it persists alongside the mitigation and business reports.
            _evr = st.session_state.get("eval_report")
            if _evr:
                if _evr.get("error"):
                    st.error(_evr["error"])
                else:
                    st.markdown(_evr["markdown"])
                    st.caption(f"Generated on-box by `{_evr['model']}`.")
            ...
            if strategy_result is not None:
                if strategy_result.get("status") == "success":
                    st.session_state["business_report"] = {
                        "markdown": strategy_result.get("strategy_markdown", ""),
                        "model": strategy_result.get("model", "local model"),
                    }
                    logger.info(
                        "AI STRATEGY REPORT -> received %d characters from model '%s'",
                        len(st.session_state["business_report"]["markdown"]),
                        strategy_result.get("model", "?"),
                    )
                else:
                    st.session_state["business_report"] = {
                        "error": strategy_result.get("message", "AI strategy generation failed."),
                    }
                    logger.warning("AI STRATEGY REPORT -> backend returned error_type=%s", strategy_result.get("error_type"))
        # Render the business strategy report from session state on EVERY rerun, so
        # it persists alongside the mitigation memo instead of replacing it.
        _biz = st.session_state.get("business_report")
        if _biz:
            if _biz.get("error"):
                st.error(_biz["error"])
            else:
                st.markdown(_biz["markdown"])
                st.caption(f"Generated on-box by `{_biz['model']}`.")

        # ---- Simulation-vs-Real accuracy via a synthetic field trip -----------
        st.divider()
        st.subheader("🛰️ Simulation vs Real — Simulated Field Trip")
        st.caption(
            "No Traccar hardware connected, so this runs the SAME accuracy scoring against a synthetic "
            "'real' trip generated from the plan with the deviations you set below — a clearly-labelled "
            "stand-in for a live Traccar run. Validation Score = 0.5·route-adherence + 0.3·ETA-closeness "
            "+ 0.2·thermal-closeness (each 0–100)."
        )
        fcols = st.columns(3)
        _adh = fcols[0].slider("Route adherence", 0.60, 1.00, 0.90, 0.02,
                               help="Fraction of the synthetic trip's GPS pings that stay on the planned corridor.")
        _tov = fcols[1].slider("Time over plan (%)", 0, 40, 12, 1,
                               help="How much longer the synthetic real trip runs than the simulated ETA.")
        _thx = fcols[2].slider("Thermal excursion (°C over safe)", 0.0, 6.0, 0.0, 0.5,
                               help="Peak cargo temperature above the safe max on the synthetic trip.")
        if st.button("🛰️ Run simulated field-trip evaluation", use_container_width=True):
            st.session_state["field_eval"] = backend_get(
                f"/v1/simulation/trip-evaluation?adherence={_adh}&time_over_pct={_tov}"
                f"&thermal_excursion_c={_thx}", silent=True)
        _fev = st.session_state.get("field_eval")
        if _fev and _fev.get("status") == "success":
            _score = _fev["validation_score"]
            _fc = st.columns(4)
            _fc[0].metric("Validation Score", f"{_score:.0f}/100")
            _fc[1].metric("Route Adherence", f"{_fev['route_adherence_pct']:.0f}%")
            _fc[2].metric("Actual vs Planned Time",
                          f"{_fev['actual_elapsed_min']:.0f} / {_fev['planned_time_mins']:.0f} min")
            _pk = _fev.get("actual_peak_cargo_temp_c")
            _fc[3].metric("Peak Cargo Temp", f"{_pk:.1f} °C" if _pk is not None else "n/a")
            (st.success if _score >= 85 else st.warning if _score >= 60 else st.error)(
                f"Simulation accuracy for this corridor: {_score:.0f}/100 "
                f"(higher = the real trip tracked the simulation more closely).")
            if _fev.get("disclaimer"):
                st.caption(_fev["disclaimer"])
            st.markdown("**How to make the real trip match the simulation:**")
            for _s in _fev.get("suggestions", []):
                st.markdown(f"- {_s}")
            if st.button("💡 Generate AI Simulation-vs-Real Report", use_container_width=True,
                         key="field_eval_ai_btn"):
                _amb = (strategy_telematics or {}).get("ambient_weather") or {}
                _sim_text = (
                    f"Route adherence {_fev['route_adherence_pct']:.0f}%; actual "
                    f"{_fev['actual_elapsed_min']:.0f} min vs planned {_fev['planned_time_mins']:.0f} min; "
                    f"peak cargo {_pk} °C; validation {_score:.0f}/100. "
                    + " ".join(_fev.get("suggestions", []))
                )
                _time_pct = round(_fev["time_ratio"] * 100.0, 0) if _fev.get("time_ratio") is not None else None
                _eval_payload = {
                    "origin": "Simulated field trip",
                    "destination": _fev.get("destination_town", ""),
                    "standard_time_mins": _fev["planned_time_mins"],
                    "optimized_time_mins": _fev["actual_elapsed_min"],
                    "rand_saved": 0.0,
                    "mechanical_risk_reduction_pct": 0.0,
                    "thermal_risk_pct": (strategy_telematics or {}).get("thermal_risk_pct", 0.0),
                    "cargo_temp_status": (strategy_telematics or {}).get("cargo_temp_status", "Unknown"),
                    "surface_profile": _fev.get("planned_surface_profile") or "unknown",
                    "shipment_value_rand": float(st.session_state.shipment_value_rand),
                    "ambient_temp_c": _amb.get("temp_c"),
                    "rain_mm": _amb.get("rain_mm"),
                    "weather_alert": _amb.get("alert"),
                    "evaluation_mode": "simulated_field_trip",
                    "validation_score": _score,
                    "route_adherence_pct": _fev["route_adherence_pct"],
                    "actual_vs_planned_time_pct": _time_pct,
                    "sim_vs_real_text": _sim_text,
                }
                with st.spinner("🤖 Local model compiling the simulation-vs-real evaluation... "
                                "(first run ~2-4 min on CPU, then fast)"):
                    _er = backend_post("/v1/analytics/strategy", json_body=_eval_payload, timeout=600)
                if _er is not None:
                    if _er.get("status") == "success":
                        st.session_state["eval_report"] = {
                            "markdown": _er.get("strategy_markdown", ""),
                            "model": _er.get("model", "local model"),
                        }
                    else:
                        st.session_state["eval_report"] = {
                            "error": _er.get("message", "AI evaluation report failed."),
                        }
        elif _fev and _fev.get("status") in ("pending", "unavailable"):
            st.info(_fev.get("message", ""))
        # persisted sim-vs-real AI report (shared slot with the hardware path)
        _evr = st.session_state.get("eval_report")
        if _evr:
            if _evr.get("error"):
                st.error(_evr["error"])
...
with tab_weather:
    ...
    # Persistent slots created ONCE + claimed with an initial value, so the fragment
    # updates ONLY their values every 30s (Current Position tracks the moving truck)
    # without rebuilding the layout — no flicker. Static headers are created here too.
    ...
    # initial claim render — writes every slot once during the main run
    wui["source_info"].caption(" ")
    wui["climate_header"].subheader("Active Vehicle Climate Context")
    wui["temp_metric"].metric("Outside Ambient Temperature", "-")
    wui["rain_metric"].metric("Rain Index", "-")
    wui["warm_metric"].metric("Warming Pressure", "-")
    wui["climate_alert"].caption(" ")
    wui["forecast_empty_state"].info("Waiting for the trip's route...")
    wui["forecast_table"].caption(" ")
    wui["forecast_alert"].caption(" ")
    wui["hardware_note"].caption(" ")
    wui["source_footer"].caption(" ")

    # Poll often enough that Current Position tracks the moving truck; in-place
    # updates mean this no longer flickers even though the weather lookup is slow.
    @st.fragment(run_every=5)
    def weather_tab_view():
        # Only UPDATE the existing slots' values each poll — no layout rebuild.
        ...
        # Build the source note as ONE string and write it to the slot in place
        # (no .container() on an outside slot inside the fragment → no crash/flicker).
        ...
        wui["source_info"].caption(_src_note)
        ...
        if weather_profile is None or not weather_profile.get("segments"):
            ...
            wui["forecast_alert"].markdown(" ")
            wui["hardware_note"].markdown(" ")
            wui["source_footer"].markdown(" ")
            return
        wui["forecast_empty_state"].markdown(" ")
