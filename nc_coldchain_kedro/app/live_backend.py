...
$env:NC_ROAD_GRAPH="data\08_graph\optimized\nc_road_graph.pkl"
...
# --- Kedro integration bootstrap ------------------------------------------------
# Make the vendored domain modules (nc_road_network_improved, nc_boundary,
# weather_engine, vrp_simulation_generator) importable wherever uvicorn is
# launched from, by adding the package's legacy/ dir to sys.path.
import sys as _sys
_LEGACY = os.path.join(os.path.dirname(__file__), "..", "src", "nc_coldchain", "legacy")
if os.path.isdir(_LEGACY) and _LEGACY not in _sys.path:
    _sys.path.insert(0, _LEGACY)
# -------------------------------------------------------------------------------
...
# Configuration
# ---------------------------------------------------------------------------
# Loads the graph the Kedro pipeline produced (data/08_graph/optimized/nc_road_graph.pkl).
# Override with NC_ROAD_GRAPH; falls back to a local nc_road_graph.pkl if present.
GRAPH_PATH = Path(os.environ.get(
    "NC_ROAD_GRAPH",
    os.path.join(os.path.dirname(__file__), "..", "data", "08_graph",
                 "optimized", "nc_road_graph.pkl"),
))
if not GRAPH_PATH.exists() and Path("nc_road_graph.pkl").exists():
